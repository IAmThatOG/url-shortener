...
export class UrlService implements IUrlService {
  ...
  encodeUrl(requestDTO: EncodeUrlRequestDto): BaseResponseDto {
    ...
    return <ServiceResponseDto<EncodeUrlResponseDto>>{
      status: true,
      statusCode: HttpStatus.CREATED,
      payload: { shortUrl: `${URL_DOMAIN}/${url.shortCode}` },
    };
  }
  ...
  decodeUrl(shortUrl: string): BaseResponseDto {
    ...
    const url = this.urlRepo.FetchSingle((x) => x.shortCode == shortCode);
    if (!url) {
      throw new NotFoundException({ code: ResponseCode.UrlNotFound });
    }
    url.visitLongUrl();
    this.urlRepo.Update(url);
    return <ServiceResponseDto<DecodeUrlResponseDto>>{
      status: true,
      statusCode: HttpStatus.TEMPORARY_REDIRECT,
      payload: { longUrl: url.longUrl },
    };
  }
  ...
  getStatistics(urlPath: string): BaseResponseDto {
    ...
    const url = this.urlRepo.FetchSingle((x) => x.shortCode == urlPath);
    if (!url) {
      throw new NotFoundException({ code: ResponseCode.UrlNotFound });
    }
    return <ServiceResponseDto<UrlStatisticsResponse>>{
      status: true,
      statusCode: HttpStatus.OK,
      payload: {
        lastVisited: url.lastVisit,
        numberOfVisits: url.numberOfVisit,
      },
    };
  }
}
